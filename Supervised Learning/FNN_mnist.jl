...
# ╔═╡ f3d9f006-07e6-4387-a8b7-57bf6535eccd
using Statistics

# ╔═╡ 1e2704cb-39f2-4326-b8e4-f5c8151cdf4e
using Flux

# ╔═╡ 9b1134bf-bcfd-434a-af8c-b1eb35cdbcce
using Plots

# ╔═╡ 5dae16d8-639a-43b5-9fab-005b389618a1
using Optim

# ╔═╡ 5070f1c8-9ffa-43f0-a7d4-c5553d64d2ff
using ImageShow

# ╔═╡ 1079cf5b-5bf2-4f3f-bec1-a750e527c69d
using ImageIO

# ╔═╡ 06e77713-e58c-4ed9-bf14-983bed89f0ff
using MLDatasets

# ╔═╡ 9d58810e-35f7-4a9b-83d4-33acf1cd505e
using MLDatasets: MNIST

# ╔═╡ 0a038e4e-23c9-47d8-b215-0312c86c8173
using DataFrames

# ╔═╡ f28bcf99-72ff-424f-8b60-86d88ff9f14d
dataset = MNIST(:train)

# ╔═╡ a4bd36e8-20ee-4b9a-a06b-69acc038c548
X = dataset.features[:,:,1:2000]

# ╔═╡ e8a03f54-b42d-4636-9d88-cfbbe8f06fbf
x1 = X[:,:,1]

# ╔═╡ be493311-002b-45de-894e-513dc0cb3265
i1 = MLDatasets.convert2image(MNIST, X[:,:,3])

# ╔═╡ 9df99272-1ffe-4769-88bb-00f620f2ba51
layer1 = Dense(5 => 3)

# ╔═╡ 77d708ff-9556-4ea5-a1b1-cb34253650ed
layer1.bias

# ╔═╡ 5e85a692-0d42-4a05-a703-8ade35c0be10
layer1.weight

# ╔═╡ cf7b1ea6-16b3-42c9-9ad8-2da5c58582bc
layer1.σ

# ╔═╡ 7ecfe5aa-260f-4295-8131-981d18918629
u = Float32.(rand(5))

# ╔═╡ 97348bfa-09ca-4879-a966-1d59c4b13e3c
layer1(u)

# ╔═╡ fd290a28-4e01-4786-a6b6-30b6e90493c0
layer1.weight * u + layer1.bias

# ╔═╡ b8867bfd-1812-4847-bc4c-18927b1a1398
layer1.σ(layer1.weight * u + layer1.bias)

# ╔═╡ 4f0d6c65-4e1f-456a-9ae4-13510ac1c737
layer2 = Dense(5 => 2, σ)

# ╔═╡ 5f4f4435-f9db-49d6-983d-9b0d5a5a37d9
layer2(u)

# ╔═╡ 553f00b5-ef21-4c3d-b599-cb2be89da699
layer2.σ(layer2.weight * u + layer2.bias)

# ╔═╡ 61a8ef87-29f7-4664-8fd6-8f27949ad952
l1 = Dense(28 * 28, 64, tanh)

# ╔═╡ 123410ae-e901-4031-8911-e0fa455d6aaa
a1 = l1(x1[:])

# ╔═╡ bf4f6e9f-ff4f-4386-9fcb-09f7fad22eca
l2 = Dense(64 => 32, relu)

# ╔═╡ c9904851-7d43-40bb-ae70-f839bb50bb3f
a2 = l2(a1)

# ╔═╡ 33f1b904-b111-4dc0-8579-0a75f4d8194d
network = Chain(l1, l2, Dense(32 => 10), softmax)

# ╔═╡ 3169dfd1-8380-4ff6-bdff-81b94cd625cb
argmax(network(x1[:])) - 1

# ╔═╡ 65713140-a246-44e5-abfb-29c8ab0d8868
argmax(network(X[:,:,2][:])) - 1

# ╔═╡ db93da47-3bf1-4819-807b-0cccbc179bd1
a = rand(3)

# ╔═╡ f6e85b35-1150-4c1e-b6cb-064d9277af63
b = rand(3)

# ╔═╡ fa688e0f-d603-4760-ac16-e4e141de18d6
Flux.logitcrossentropy(a, b)

# ╔═╡ 1d3a2cda-afaf-466d-b2de-7e210ff98a89
Flux.crossentropy(a, b)

# ╔═╡ 73c23a6f-f213-465e-a6ba-7d5af32fe4fa
y = dataset.targets[1:2000]


# ╔═╡ ca5daf0b-d408-4fec-96a9-29bb15a4f2bd
ŷ1 = network(x1[:])

# ╔═╡ 6e993f84-e687-42b5-89cd-fb412976123c
y1 = Flux.onehot(y[1], 0:9)

# ╔═╡ 8b3d69ee-7474-4994-bb36-0c4428bb9556
loss(ŷ, y) = Flux.crossentropy(ŷ, y)

# ╔═╡ 4fbcec09-7f8e-4db8-a1a9-1406342b3626
loss(ŷ1, y1)

# ╔═╡ b3e2eb79-57a3-425f-b044-e742ed72e7c7
loss(network(X[:,:,2][:]), Flux.onehot(y[2], 0:9))

# ╔═╡ 2510f719-81af-4f4f-8643-bc59c711305f
X8 = Float32.(zeros(28 * 28, 8))

# ╔═╡ 841eaa2e-22dc-4132-85f2-d4bbd6038066
for i=1:8
	X8[:, i] .= X[:,:,i][:]
end

# ╔═╡ 84b416e4-98e4-43a2-afc5-c4a1988b4048
sum(X8, dims=1)

# ╔═╡ 1a668d70-9c26-4738-9a6b-4cfd7408625d
l1(X8)

# ╔═╡ 40dc54fd-bb7e-440b-b1d6-38771a0ad87f
ŷ8 = network(X8)

# ╔═╡ fa77c437-1dd3-4cfc-84ff-153c973e987d
y8 = Flux.onehotbatch(y[1:8], 0:9)

# ╔═╡ 38a2423c-b679-4c82-9bce-d9e6e904bda3
loss(ŷ8, y8)

# ╔═╡ d69c49c8-626d-4533-a55d-6f26aa32935a
ŷ = [network(X[:,:,i][:]) for i = 1:2000]

# ╔═╡ 98f54f5e-af58-42c6-935d-5acd01a48634
target = Flux.onehotbatch(y, 0:9)

# ╔═╡ 853e5fa7-5396-4779-8b14-726263184175
X_train = [X[:,:,i][:] for i = 1:2000]

# ╔═╡ c5d6687d-0f01-450f-9ed3-2905ae85ffe9
y_train = [Flux.onehot(y[i], 0:9) for i = 1:2000]

# ╔═╡ 824b512f-9d23-4b95-b238-65e77ef5e1c0
data = zip(X_train, y_train)

# ╔═╡ 36e6d346-d09b-4c66-8397-9185e4791c14
optimizer = Adam() #Thuat toan toi uu Adam giong Newton

# ╔═╡ f4b000eb-bb9d-4e19-bd08-d77e1c03adc3
θ = Flux.params(network) #Lấy tất cả các parameters bao gồm cả bias và weight

# ╔═╡ d3a683cb-0ad4-4c8b-b14f-e8e43583b26f
cost(model, x, target) = Flux.crossentropy(model(x),target)

# ╔═╡ c2afe9c2-2967-43ff-aee8-fdfa3a86722c
opt_state = Flux.setup(optimizer, network)

# ╔═╡ cd4178ce-f77c-4dcb-81cd-5c995d43089b
Flux.train!(cost, network, data, opt_state)

# ╔═╡ 296f00c7-dfe1-47b8-a1b3-25d9cd85f07b
Flux.@epochs 10 Flux.train!(cost, network, data, opt_state)

# ╔═╡ 310518f6-d6ed-436d-94fa-ebdee24abb81
pred = [Flux.onecold(network(X_train[i])) for i=1:2000] .- 1

# ╔═╡ 5094f425-d9b2-45da-9ef0-cce60e2b0e1e
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "64dfb266d32c2ee129b855762a751204392848b5"
