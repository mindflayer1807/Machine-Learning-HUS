...
# ╔═╡ 674a50e2-0e19-4f10-a081-5f108bda0350
using MLDatasets: MNIST

# ╔═╡ 6b9aa764-0380-43cd-99a2-c924f9e0dd9f
using ImageShow

# ╔═╡ bc8ea831-64dc-405a-9b9b-95a7ce14ee9a
using ImageIO

# ╔═╡ c73baba7-b3bc-4eb0-b76c-8ec805da449f
using MLDatasets

# ╔═╡ 6651d771-d015-46be-8b14-904ac4e1ee78
using Flux

# ╔═╡ 9f6e3154-bce1-4ccf-95f3-c2b114954aee
dataset = MNIST(:train)

# ╔═╡ daaa749c-6292-4f85-89d6-c4280c76e202
X = dataset.features[:,:,1:2000]

# ╔═╡ 5239ba97-b476-4c88-a7eb-c4abbbb6e346
y = dataset.targets[1:2000]

# ╔═╡ f0a85f48-7af4-4e02-8f6c-e2e36bf168d3
x1 = X[:,:,1]

# ╔═╡ 68e277e0-b7f5-4eac-97f4-eb23683f1fd4
convert2image(MNIST(), x1)

# ╔═╡ 18e80936-0068-4caa-9d25-e7a1e620f38c
[convert2image(MNIST(),X[:,:,i]) for i = 1:5]

# ╔═╡ c002fbfe-2fe1-4a42-9a06-fc517f1bf766
layer1 = Dense(5 => 3)

# ╔═╡ 5a0584bb-0f5d-44e8-b91c-e7df42bd7e3d
layer1.bias

# ╔═╡ 4d7f785a-9b24-4d94-89be-9bd88b1a02ec
layer1.weight

# ╔═╡ f74b8301-8024-4a4d-8459-ffdb45d5688f
layer1.σ

# ╔═╡ 9724c1df-a7f7-45fd-b380-9e3af8b629be
u = Float32.(rand(5))

# ╔═╡ 928e0e5c-c3f8-42e9-a970-a61dcff90a4e
layer1(u)

# ╔═╡ 170ba546-182a-4e1a-86b6-de4fcb5e3b21
layer1.weight*u + layer1.bias

# ╔═╡ a1af5eb2-b13a-4c49-a642-107b135276c9
layer2 = Dense(5 => 2, σ)

# ╔═╡ 109db70b-7d49-4fdc-a476-c203ed633ee9
layer2(u)

# ╔═╡ 7c6e9fc7-498b-441c-a657-66318f2e4f09
layer2.:σ(layer2.weight*u + layer2.bias)

# ╔═╡ 8ec04762-8c0e-4ddc-9bbe-bbffe75c0119
l1 = Dense(28*28 => 64, tanh)

# ╔═╡ c252a5b5-5385-4a46-8833-c99f92dd3e66
a1 = l1(x1[:])

# ╔═╡ 5abc5684-6e8f-4503-9ffd-642c6f31a013
l2 = Dense(64 => 32, relu)

# ╔═╡ 43595770-0807-459e-b6e7-339c54aa51f5
a2 = l2(a1)

# ╔═╡ ab73446c-f6aa-42f4-99fa-532f7c77291f
network = Chain(l1, l2, Dense(32 => 10), softmax)

# ╔═╡ d37fcef5-3963-4947-85ef-1ef7275746ed
ŷ1 = network(x1[:])

# ╔═╡ d66d0d9b-5649-4d67-a198-6ea3c12d7756
ŷ2 = network(X[:,:,2][:])

# ╔═╡ 343feffd-c350-4c08-8ab9-a8bf4066e726
# ╠═╡ disabled = true
#=╠═╡
y = dataset.targets
  ╠═╡ =#

# ╔═╡ c3d18adc-3a01-49fe-9e27-d67323ed766d
y2 = Flux.onehot(y[2], 0:9)

# ╔═╡ 032655ce-b232-44d7-b3b5-5ff4d0fc3734
loss(ŷ, y) = Flux.crossentropy(ŷ, y)

# ╔═╡ 05397718-579e-4592-a24c-3911416fd0be
loss(ŷ2, y2)

# ╔═╡ 3b262086-ee62-4ad3-ae95-235741dd2927
#X8 = X[:,:,1:8]

# ╔═╡ ff93b54c-fe50-4e47-bcaf-361ccea98257
#X8 = zeros(X[:,:,1].size,8)
# for i = 1:8
# 	X8[:,i] = X[:,:,i][:]
# end

# ╔═╡ 6ce80aa1-c724-4dea-8666-fb699349b26c
X8 = reshape(X[:,:,1:8], (28*28,8))

# ╔═╡ ea766380-a341-4e4c-b881-c3236559febf
sum(X8, dims = 1)

# ╔═╡ 8aa1916b-8352-4415-b67e-5c277d253d47
ŷ8 = network(X8)

# ╔═╡ 1e138d3a-32f3-4e96-b9a4-d9eb42dd4bb2
y8 = Flux.onehotbatch(y[1:8],0:9)

# ╔═╡ 676ba6cc-1d10-4e57-995b-08df010adfac
loss(ŷ8, y8)

# ╔═╡ 962e3518-cc72-4eb3-a253-0f8828599a75
ŷ = [network(X[:,:,i][:]) for i = 1:2000]

# ╔═╡ 7e35b561-1deb-4660-9fdf-a6db86adba98
# ╠═╡ disabled = true
#=╠═╡
predict = Flux.stack(ŷ)
  ╠═╡ =#

# ╔═╡ 4c645b6a-b69e-42c4-bcc0-9f1c5997dedf
target = Flux.onehotbatch(y[1:2000], 0:9)

# ╔═╡ 90b56cf6-f12c-47d9-9c38-f7a0f8451590
#=╠═╡
loss(predict, target)
  ╠═╡ =#

# ╔═╡ c666382a-abc0-4cf2-912a-bc0f4cace96b
#=╠═╡
sum(Flux.onecold(predict) .== y) / length(y)
  ╠═╡ =#

# ╔═╡ 5cacfbb4-f840-4f9b-a713-ed59aed1427c
X_train = [X[:,:,i][:] for i = 1:2000]

# ╔═╡ 245b9e72-1801-476f-a8c9-31b0b83426e8
y_train = [Flux.onehot(y[i],0:9) for i=1:2000]

# ╔═╡ b384eab6-0833-486d-ae34-ab37a42d1595
data = collect(zip(X_train, y_train))

# ╔═╡ 74c4938a-48e8-40a2-a14a-ac2f187064b1
optimizer = Adam()

# ╔═╡ aa701836-9df1-46da-b820-b1a5122ca237
opt_state = Flux.setup(optimizer, network)

# ╔═╡ ed62a9e9-2d0b-4a4a-b35e-c27528c9e1ec
cost(model, x, target) = Flux.crossentropy(model(x), target)

# ╔═╡ 643413af-4188-4fde-9ff8-b7b28a3b559d
Flux.@epochs 10 Flux.train!(cost, network, data, opt_state)

# ╔═╡ b676237c-b82c-41f2-a37e-042989902106
z = [Flux.onecold(network(X_train[i])) for i = 1:2000]

# ╔═╡ 1c55e89f-eca7-412d-9e96-51e81bbd808f
sum(z .== ( y .+ 1)) / 2000

# ╔═╡ bc731120-d29d-432e-ba88-fcef8eb93d1c
